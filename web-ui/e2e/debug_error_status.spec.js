...
test.describe('Debug Error Status', () => {
  test('error status updates history sidebar', async ({ page }) => {
    ...
    // Wait for "Check failed" to appear in the FIRST history item (not just anywhere on page)
    let success = false;
    for (let i = 0; i < 30; i++) {
      await page.waitForTimeout(1000);
      
      // Get the first history item's stats line text
